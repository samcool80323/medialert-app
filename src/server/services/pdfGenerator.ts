...
export class PDFGenerator {
  ...
  private generateFallbackPDF(data: any): Buffer {
    // Generate a simple text-based PDF content as fallback
    const textContent = `
MEDIGUARD AI - AHPRA COMPLIANCE REPORT

Website: ${data.url}
Scan Date: ${new Date(data.scanDate).toLocaleDateString('en-AU')}

SUMMARY:
- Total Violations: ${data.summary.totalViolations}
- Critical: ${data.summary.criticalViolations}
- High: ${data.summary.highViolations}
- Medium: ${data.summary.mediumViolations}
- Low: ${data.summary.lowViolations}

VIOLATIONS:
${data.violations.map((v: any, i: number) => `
${i + 1}. ${v.rule} (${v.severity.toUpperCase()})
   Issue: ${v.issue}
   Original Text: "${v.originalText}"
   Suggestion: ${v.suggestion}
`).join('\n')}

Generated by MediGuard AI - ${new Date().toLocaleDateString('en-AU')}
    `;
    
    // Return as simple text buffer (in real implementation, you'd use a PDF library like PDFKit)
